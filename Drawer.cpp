...
void Drawer::DrawFourSpecters(vector<double> spectr, vector<double> rect, vector<double> trian, vector<double> rofl, double fd, int wlen)
{
	if (!init) return;
	...
	memDC.MoveTo(convert_range((wlen/2) * len, actual_right, actual_left, data_x_max, data_x_min), rect[0]);
	for (int i = 0; i < rect.size(); i++)
	{
		double x = convert_range((wlen/2 + i) * len, actual_right, actual_left, data_x_max, data_x_min);
	...
	memDC.MoveTo(convert_range((wlen / 2) * len, actual_right, actual_left, data_x_max, data_x_min), trian[0]);
	...
	memDC.MoveTo(convert_range((wlen / 2) * len, actual_right, actual_left, data_x_max, data_x_min), rofl[0]);
	for (int i = 0; i < rofl.size(); i++)
	{
		double x = convert_range((wlen / 2 + i*wlen) * len, actual_right, actual_left, data_x_max, data_x_min);
...
void Drawer::DrawDispersionOnce(double rect, double trian, double rofl, double nl)
{
	if (!init) return;
	...
	double data_x_min = 0.99 * nl;
	double data_x_max = 1.01*nl;
	double data_y_min = 0.75 * min(rect, min(trian, rofl));
	double data_y_max = 1.25 * max(rect, max(trian, rofl));
	...
	memDC.MoveTo(actual_left, convert_range(rect, actual_top, actual_bottom, data_y_max, data_y_min));
	memDC.LineTo(actual_right, convert_range(rect, actual_top, actual_bottom, data_y_max, data_y_min));
	memDC.SelectObject(&data_pen3);

	memDC.MoveTo(actual_left, convert_range(trian, actual_top, actual_bottom, data_y_max, data_y_min));
	memDC.LineTo(actual_right, convert_range(trian, actual_top, actual_bottom, data_y_max, data_y_min));
	memDC.SelectObject(&data_pen4);

	memDC.MoveTo(actual_left, convert_range(rofl, actual_top, actual_bottom, data_y_max, data_y_min));
	memDC.LineTo(actual_right, convert_range(rofl, actual_top, actual_bottom, data_y_max, data_y_min));
...
void Drawer::DrawDispersion(vector<double> rect, vector<double> trian, vector<double> rofl, double nmin, double nmax)
{
	...
	int N = rect.size();
	double data_x_min = nmin;
	double data_x_max = nmax;
	double data_y_min = 0;
	double data_y_max = max(
	...
	double len = (nmax - nmin) / (rect.size() - 1);
	...
	for (int i = 0; i < rect.size(); i++)
	{
		double x = convert_range(data_x_min + i * len, actual_right, actual_left, data_x_max, data_x_min);
		memDC.LineTo(x, rect[i]);
	}
